...
export default function Leads() {
  ...
  const { data: stats } = useGetEstatisticasQuery(currentSchoolId);

  const [createLead, { isLoading: isCreating }] = useCreateLeadMutation();
  const [updateLead, { isLoading: isUpdating }] = useUpdateLeadMutation();
  const [deleteLead, { isLoading: isDeleting }] = useDeleteLeadMutation();
  const [mudarStatus] = useMudarStatusMutation();
  const [exportarCSV, { isLoading: isExporting }] = useExportarCSVMutation();
  ...
  const adicionarLead = async () => {
    ...
    try {
      ...
      setMensagem({ 
        tipo: 'erro', 
        texto: `❌ Erro ao adicionar lead: ${error.data?.detail || error.message || 'Erro desconhecido'}` 
      });
    }
  };
  ...
  const atualizarLead = async () => {
    ...
    try {
      ...
      setMensagem({ 
        tipo: 'erro', 
        texto: `❌ Erro ao atualizar lead: ${error.data?.detail || error.message || 'Erro desconhecido'}` 
      });
    }
  };
  ...
  const executarDelecao = async () => {
    ...
    try {
      ...
      setMensagem({ 
        tipo: 'erro', 
        texto: `❌ Erro ao deletar lead: ${error.data?.detail || error.message || 'Erro desconhecido'}` 
      });
    }
  };
  ...
  const atualizarStatus = async (id: number, novoStatus: Lead['status']) => {
    try {
      ...
      setMensagem({ 
        tipo: 'erro', 
        texto: `❌ Erro ao atualizar status: ${error.data?.detail || 'Erro desconhecido'}` 
      });
    }
  };
  ...
  const handleExportarCSV = async () => {
    try {
      const blob = await exportarCSV({ escola_id: currentSchoolId }).unwrap();
            ...
            {leadsError && (
              <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-lg">
                <div className="flex items-center gap-2 text-red-700">
                  <AlertCircle size={20} />
                  <span className="font-semibold">Erro ao carregar leads</span>
                </div>
              </div>
            )}
