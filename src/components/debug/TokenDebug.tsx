...
import { useGetInstanceStatusQuery } from '../../services/uzapiApi';
import { CheckCircle, XCircle, AlertCircle, Info } from 'lucide-react';

const TokenDebug = () => {
  const { data: schoolsData, isLoading: isLoadingSchools } = useGetSchoolsQuery();
  const { error: statusError } = useGetInstanceStatusQuery();
  ...
  const has401Error = (statusError as any)?.status === 401;
      ...
      <div className="space-y-2 text-sm">
        ...
        {has401Error && (
          <div className="flex items-start gap-2 bg-red-50 p-2 rounded">
            ...
            <div className="flex-1">
              <span className="font-medium text-red-900">Erro 401:</span>
              <p className="text-red-700 text-xs mt-1">
                Token inválido ou não autorizado. Verifique o token no painel do uazapi.
              </p>
            </div>
          </div>
        )}
